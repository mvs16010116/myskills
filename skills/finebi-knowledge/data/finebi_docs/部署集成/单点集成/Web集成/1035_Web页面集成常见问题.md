...
title: Web页面集成常见问题
doc_id: 1035
url: https://help.fanruan.com/finebi6.X/doc-view-1035.html
source: FineBI 帮助文档
crawled_at: 2026-07-02 15:11:13
---
...
* 历史版本：[52](<edition-list-1035.html>)
* 最近更新：[Carly](<user-space-222366.html>) 于 2024-12-13 
...
请求方法| 常见的请求方法为 GET 和 POST，其他的方式见下图。注：FineBI支持GET和POST两种请求方式。![](/core/style/lod.png)  
...
**1）正式使用：** 建议配置单点登录，否则嵌入式集成后，需要反复登录。详情请参见：[单点登录](<https://help.fanruan.com/finebi6.0/doc-view-59.html>)
...
| 获取示例  
---|---  
用户ID| 点击「我的分析」，链接后面的ID即为当前用户的ID![](/core/style/lod.png)  
我的分析下文件夹ID| 点击文件夹，链接后面的ID即为当前用户的ID![](/core/style/lod.png)  
我的分析下主题ID| 点击主题，链接中subject后的ID即为主题ID![](/core/style/lod.png)  
我的分析下数据表ID| 点击数据表，链接中table后的ID即为数据表ID![](/core/style/lod.png)  
我的分析下组件ID| 点击组件，链接中widget后的ID即为组件ID![](/core/style/lod.png)  
我的分析下仪表板ID| 点击仪表板，链接中report后的ID即为仪表板ID![](/core/style/lod.png)  
公共数据下文件夹ID| 点击文件夹，链接后面的ID即为文件夹ID![](/core/style/lod.png)  
公共数据下表原始名| 点击数据集，链接中table后的名称即为数据表原始名![](/core/style/lod.png)  
## 6\. 其他接口
### 6.1 跨域登录接口
作用：使用该接口可以登录 BI 系统。
...
![1574315163723542.png](/core/style/lod.png)
**解决方案：**
需要获得该页面的权限，请参考 [分级权限分配](<https://help.fanruan.com/finebi6.0/doc-view-726.html>)
### 7.2 拒绝了我们的连接请求
**问题描述**  

FineBI 通过 iframe 页面嵌入到其他页面中报错：xxx拒绝了我们的连接请求，如下图所示：
![03.png](/core/style/lod.png)
**解决方案**
...
[/code]
### 7.4 返回错误码
在进行系统集成时，如果后台有错误，会返回错误码（errorCode），具体异常码定义可以参考 [系统错误码说明](<https://help.fanruan.com/finebi6.0/doc-view-530.html>)
...
[![](/core/style/back.png)上一篇：Web页面简单集成示例](<index.php?doc-view-372.html>)
[下一篇：数据更新接口 ![](/core/style/forward.png) ](<index.php?doc-view-2041.html>)
  * 有帮助
  * 没帮助
  * 只是浏览
