...
doc_id: 2407
url: https://help.fanruan.com/finebi6.X/doc-view-2407.html
source: FineBI 帮助文档
crawled_at: 2026-07-02 15:03:14
---

> 1. 概述1.1 版本FineBI版本功能变动6.1-1.2 语法clean的对象是字段，效果是在当前 def 的计算中忽略该字段上的过滤效果。语法CLEAN(字段1,字段2,字段3....)/CLEA
...
语法  
| CLEAN(字段1,字段2,字段3....)/CLEAN("ALL")| 当前 [分析函数](<https://help.fanruan.com/finebi6.X/doc-view-1993.html>) 计算中忽略指定字段生成的所有过滤效果  
---|---|---  
...
**2）clean（all）**
当前 [分析函数](<https://help.fanruan.com/finebi6.X/doc-view-1993.html>) 运算的结果不受到任何过滤效果影响（不包括仪表板/组件外的过滤，如权限过滤和定时调度的过滤）
**3）总结**
因此，CLEAN函数作用范围如下：
...
![2024-05-08_14-02-40.png](/core/style/lod.png)
2）只想分析 7 月的订单销售数据。在过滤器中拖入「订单日期」，年月过滤，选择「2022年7月」。如下图所示：
![2024-05-08_14-08-53.png](/core/style/lod.png)
此时，我们希望增加一列历史订单单笔最大金额和7月的数据进行对比。添加计算字段「历史单笔订单最大金额」。如下图所示：
![2024-05-08_14-26-20.png](/core/style/lod.png)
场景| 公式| 描述  
---|---|---  
...
[![](/core/style/back.png)上一篇：EARLIER-获取当前行的值（只用于DEF类函数）](<index.php?doc-view-1991.html>)
[下一篇：CLEAN_WIDGET函数-清除组件过滤效果（只用于DEF类函数） ![](/core/style/forward.png) ](<index.php?doc-view-2408.html>)
  * 有帮助
  * 没帮助
  * 只是浏览
