...
title: JSON 数据集插件
doc_id: 489
url: https://help.fanruan.com/finebi6.X/doc-view-489.html
source: FineBI 帮助文档
crawled_at: 2026-07-02 15:07:31
---
...
* 历史版本：[15](<edition-list-489.html>)
* 最近更新：[Lily.Wang](<user-space-337243.html>) 于 2025-09-03 
...
安装插件方法请参见：[插件管理](<https://help.fanruan.com/finebi6.X/doc-view-459.html>)
### 2.2 定义 JSON 数据连接
1）以管理员身份进入 FineBI，点击「管理系统>数据连接>数据连接管理>新建数据连接>所有」，可添加 JSON 数据连接，如下图所示：
![](/core/style/lod.png)
...
![](/core/style/lod.png)
注：JSON 数据连接配置保存后，选中数据连接，点击右侧![](/core/style/lod.png)按钮，可修改数据连接名称。
注：如果数据量大，则需要在「网络连接」中修改连接池最大连接数。
设置项的详细说明如下表所示：
设置项|  说明  
---|---  
地址| 用于输入json文本的请求地址（支持 HTTP 和非自签名 SSL 证书的 HTTPS）地址支持后缀参数（例如：https://api.example.com/goods?page=1&size=20），注意提供默认参数值供设计的时候查询用；地址支持除报表函数和单元格计算以外的所有公式；注：支持 HTTP 和非自签名 SSL 证书的 HTTPS  
...
查询语句| 输入查询语句| $.store.book[*]查询出书店中所有的书信息，查询结果如下图所示：![](/core/style/lod.png)注：$.store.book[*] 是 JSON 的固定引用方式，代表取出 store 下 book 的全部数据  
$.store.book[*].category 取出 category 列的数据，查询结果如下图所示：![](/core/style/lod.png)  
$.store.book[?(@.price<10)]取出价格小于 10 的书籍，查询结果如下图所示：![](/core/style/lod.png)  
### 2.4 传参方式介绍
三种传参方式区别如下表所示：
模式| 传递方式  
---|---  
...
传参方式具体介绍请参见：[JSON 数据集插件](<https://help.fanruan.com/finereport/doc-view-1985.html>) 的 3.3 节内容。
## 3\. JSON 程序数据集
...
### 3.1 新建 JSON 程序数据集
以管理员身份进入 FineBI，点击「管理系统>数据连接>服务器数据集>JSON程序数据集」，数据集名称为「JSON程序数据集」，选择创建的 JSON 数据连接，输入脚本语句，点击「保存」按钮即可。如下图所示：
![](/core/style/lod.png)
### 3.2 取出所有语句
获取所有书籍的数据，脚本语句如下所示：
[code]
...
![](/core/style/lod.png)
## 4\. 使用JSON数据集/程序数据集
1）以管理员身份进入FineBI，[添加数据库表](<https://help.fanruan.com/finebi6.X/doc-view-887.html>)  
...
[![](/core/style/back.png)上一篇：阿里云Hologres数据连接](<index.php?doc-view-1347.html>)
[下一篇：新SAP BW数据集插件 ![](/core/style/forward.png) ](<index.php?doc-view-256.html>)
  * 有帮助
  * 没帮助
  * 只是浏览
