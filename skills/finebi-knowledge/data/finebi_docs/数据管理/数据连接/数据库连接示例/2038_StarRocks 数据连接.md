...
doc_id: 2038
url: https://help.fanruan.com/finebi6.X/doc-view-2038.html
source: FineBI 帮助文档
crawled_at: 2026-07-02 15:06:57
---
...
## 3\. 具体连接操作
1）点击「管理系统>数据连接>数据连接管理>新建数据连接」，如下图所示：  
...
StarRocks 的 Catalog（数据目录）可用于访问内、外部源的数据（可参见：[catalog 简介](<https://docs.starrocks.io/zh/docs/data_source/catalog/catalog_overview/>)）。每个 StarRocks 集群都有且只有一个 internal catalog 名为 default_catalog。
  ...
  * 如下图，在 catalog 中填写一个外部数据目录 hive_test， 就可以连接到 hive_test ，并使用 [添加数据库表](<https://help.fanruan.com/finebi6.X/doc-view-887.html?source=4>) 直接从中取数 。


![](/core/style/lod.png)
![icon](/core/style/lod.png)提示:
catalog 中的表不支持和外部 Excel 表联合分析，若要进行联合分析请先配置加速引擎，详细请参见：[加速引擎](<https://help.fanruan.com/finebi6.X/doc-view-2258.html>)
4）点击「测试连接」，成功后「保存」。如下图所示：
![](/core/style/lod.png)
## 4\. 添加数据库的表至 FineBI
有两种方式可以将数据库中的表添加至 FineBI ：
![](/core/style/lod.png)
1）[添加数据库表](<https://help.fanruan.com/finebi6.X/doc-view-887.html?source=4>)
2）[添加SQL数据集](<https://help.fanruan.com/finebi6.X/doc-view-890.html>)
...
[![](/core/style/back.png)上一篇：数据连接支持范围](<index.php?doc-view-685.html>)
[下一篇：数据连接 Kerberos 认证 ![](/core/style/forward.png) ](<index.php?doc-view-282.html>)
  * 有帮助
  * 没帮助
  * 只是浏览
