...
doc_id: 293
url: https://help.fanruan.com/finebi/doc-view-293.html
source: FineBI 7.X 帮助文档
crawled_at: 2026-07-16 17:25:44
version: "7.X"
---

> 1.&nbsp;概述1.1 版本FineBI 版本功能变动6.0-7.0.11「连接池设置」中新增「初始化SQL」设置项，可配置初始化 SQL 语句1.2 应用场景本文将介绍如何连接 Apache Im

![](./view/finebi70_2025/images/info_fill.png) 您正在浏览的是 FineBI7.X 帮助文档，点击跳转至： [FineBI6.X帮助文档](<https://help.fanruan.com/finebi6.X/>)
# Apache Impala数据连接
[__](<doc-edit-293.html>)
对此内容反馈
* _
__
...
支持的数据库版本 | 驱动包链接  
---|---  
Impala 2.2Impala 2.3Impala 2.8Impala 2.9Impala 2.10| [ImpalaJDBC41.zip](<doc-download-/finebi6.X/uploads/file/20240523/ImpalaJDBC41.zip> "下载资料") （用于仅加载当前方式上传驱动）[ImpalaJDBC41.zip](<doc-download-/finebi/uploads/file/20260109/ImpalaJDBC41.zip> "下载资料")（用于优先加载当前方 式上传驱动）  
...
![Snipaste_2026-01-09_15-13-02.png](https://help.fanruan.com/core/style/lod.png)
![2026-01-09_15-41-48.png](https://help.fanruan.com/core/style/lod.png)
### 2.2 收集连接信息
在连接数据库之前，请收集以下信息：
  * 数据库所在服务器的 IP 地址和端口号；
  * 数据库的名称；
...
Kerberos 认证方式详情可参见：[数据连接 kerberos 认证](<https://help.fanruan.com/finebi7.0/doc-view-282.html?source=4>)
![](https://help.fanruan.com/core/style/lod.png)
各设置项说明如下表所示：
设置项  
| 说明  
---|---  
认证相关说明| 若 Impala 数据库存在认证，则需要在 URL 后加入AuthMech参数，不同参数值代表了不同的认证，如下表所示： 注：若数据库没有用户名密码时不需要加该参数。| 值  
| 含义  
---|---  
...
例如：jdbc:impala://192.168.5.127:21050/default;AuthMech=1;KrbHostFQDN=quickstart.cloudera;KrbServiceName=impala，具体请参见 [[平台]数据连接Kerberos认证](<http://help.finereport.com/doc-view-2556.html>)  
初始化SQL| 7.0.11 及之后版本，新增该设置项，可配置初始化 SQL 语句；详情请参见：[配置数据连接入门](<https://help.fanruan.com/finebi/doc-view-94.html>)  
Kudu 地址| 若写入到 Apache Impala 数据库，需要配置该设置项指定 Kudu 的 Master 地址，格式为 hostname:port，如果有多个 Master，用逗号隔开注：Kudu 地址里不能配置 IP，要配置 hostname 才可以正常连接。在配置 Kudu 地址前，需要在 FineReport 所在服务器的系统 hosts 文件里，为所有 Kudu 服务器配置 hostname 和 IP 的对应关系。
  * 如果 FR 所在服务器是 Windows，在C:\Windows\System32\drivers\etc的hosts文件中添加对应的映射关系
  * 如果 FR 所在服务器 Linux，在/etc/hosts文件里，添加对应的映射关系

注：如果是运维平台部署的FineDataLink，host 设置是通过运维平台的开发者模式进行配置，详情参见：[host设置](<https://help.fanruan.com/fineops/doc-view-202.html#e8eb469cd421a1b4>)文件里的映射配置示例： ![40.png](https://help.fanruan.com/core/style/lod.png)  
配置数据连接，填写 host 映射后的内容：![41.png](https://help.fanruan.com/core/style/lod.png)  
...
升级后 
**解决方案：**
将 Kerberos 认证的方式更改为第二种，详情可参见：[Kerberos 认证方式二](<https://help.fanruan.com/finebi7.0/doc-view-282.html#5>)
### 附件列表 
  
下载次数：：0
    
**主题：** [数据治理](<category-view-285>)
[![](https://help.fanruan.com/core/style/back.png)上一篇：Pivotal Greenplum Database数据连接](<index.php?doc-view-289.html>)
[下一篇：达梦DM数据连接 ![](https://help.fanruan.com/core/style/forward.png) ](<index.php?doc-view-1712.html>)
  * 有帮助
  * 没帮助
  * 只是浏览
