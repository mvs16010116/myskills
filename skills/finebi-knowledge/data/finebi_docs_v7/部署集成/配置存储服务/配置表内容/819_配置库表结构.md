...
title: 配置库表结构
doc_id: 819
url: https://help.fanruan.com/finebi/doc-view-819.html
...
* 历史版本：[76](<edition-list-819.html>)
* 最近更新：[Carly](<user-space-222366.html>) 于 2026-07-03 
...
---|---|---|---  
ID| 主键| VARCHAR| 255  
NAME| 属性名称，区分平台使用用户类型user_product_type.platform.bi_datamining：超管user_product_type.platform：PC端使用用户user_product_type.platform.bi_view：BI使用用户-查看用户user_product_type.platform.bi_design：BI使用用户-设计用户user_product_type.mobile：移动端使用用户password_change_time：密码修改时间| VARCHAR| 255  
...
### 3.6 fine_authority_apply 权限申请记录表
字段名称  
| 字段说明| 字段类型| 字段长度  
---|---|---|---  
id| 主键，申请ID| varchar| 255  
applicant| 申请人| varchar| 255  
applyExtraInfo| 扩展信息（存储字段信息等）| longtext|   
  
applyReason| 申请原因| varchar| 4000  
applyTime| 申请时间| datetime|   
  
authorityType| 申请权限类型| varchar| 255  
resourceId| 申请资源ID| varchar| 255  
resourceType| 申请资源类型默认值-1| int|   
  
status| 申请状态| int|   
  
### 3.7 fine_authority_review 权限审批记录表
字段名称  
| 字段说明| 字段类型| 字段长度  
---|---|---|---  
id| 主键，申请ID  
| varchar| 255  
reviewExtraInfo| 扩展信息（存储字段信息等）| longtext|   
  
reviewOpinion| 审批意见| varchar| 4000  
reviewTime| 审批时间| datetime|   
  
reviewer| 审批人| varchar| 255  
snapshot| 快照| longtext|   
  
### 3.8 fine_authority_review_m
字段名称  
| 字段说明| 字段类型|   
...
FILECLEARCOUNT| 附件清理数目-1：不清理0：任务结束即清理1：保留一次其他正整数：自定义保留x次| INTEGER| 32  
...
STOPONERROR| FineBI6.1.1新增功能「模板报错中断执行」false：不勾选true：勾选| BOOLEAN| 0  
...
USEATTACH| 使用模板附件| BOOLEAN| 0  
USEWIDGETATTACH| 使用组件预警附件| BOOLEAN| 0  
ID| 主键关联表字段fine_base_output>id| VARCHAR| 255  
...
### 16.4 FINE_REPORT_WARNING 仪表板配置预警信息表
字段名  
| 描述| 数据类型| 长度  
---|---|---|---  
ID| 主键ID| VARCHAR| 255  
...
### 16.5 FINE_STATISTIC_OPERATOR 直连自助数据集统计信息表
字段名  
| 描述| 数据类型| 长度  
---|---|---|---  
ID| 主键| VARCHAR| 255  
...
### 16.6 FINE_STATISTIC_TABLE 直连基础数据集统计信息表
字段名  
| 描述| 数据类型| 长度  
---|---|---|---  
ID| 主键| VARCHAR| 255  
...
### 16.7 FINE_STATISTIC_TABLE_INFO 直连自助数据集步骤信息表
字段名  
| 描述| 数据类型| 长度  
---|---|---|---  
ID| 主键| VARCHAR| 255  
...
### 16.9 FINE_UPDATE_TASK_DETAIL 更新任务明细表
字段名  
| 描述| 数据类型| 长度  
---|---|---|---  
ID| 主键| VARCHAR| 255  
...
### 16.10 FINE_UPDATE_DETAIL_INFO 更新任务明细信息表
字段名  
| 描述| 数据类型| 长度  
---|---|---|---  
...
### 16.11 FINE_FAVORITE_REPORT 分享的仪表板收藏记录表
字段名  
| 描述| 数据类型| 长度  
---|---|---|---  
ID| 主键ID| VARCHAR| 255  
...
### 16.12 FINE_PACK_FILTER BI业务包行过滤器表
...
### 16.13 FINEBI_PUBLISH_TASK
字段名称| 描述| 字段类型| 字段长度  
---|---|---|---  
id| 主键  
| varchar| 255  
creator| 资源创建者  
关联的表字段fine_user>id| varchar| 255  
details| 发布信息ID| varchar| 1000  
itemId| 发布资源ID| varchar| 255  
itemType| 发布资源类型33：仪表板3：数据集| int|   
  
publishTaskId| 发布任务ID| varchar| 255  
status| 发布状态1：申请发布2：申请取消发布3：已发布| int|   
...
---|---|---|---  
id| 主键| varchar| 255  
commentValue| base64编码记录| varchar| 1023  
deviceType|   
| int|   
...
### 16.15 fine_query_filter_condition
仪表板查询参数业务关系表存储
字段名称  
| 字段说明| 字段类型| 字段长度  
---|---|---|---  
id| 主键  
| varchar| 255  
createTime| 创建时间| bigint|   
  
dashboardId| 查询参数所归属的仪表板ID| varchar| 255  
defaultStatus| 是否为默认查询参数1代表为是0代表为否| int|   
  
filterParameters| 查询参数的具体过滤参数存储json串| longtext|   
  
filterViewId| 查询参数唯一ID| varchar| 255  
filterViewName| 查询参数名称| varchar| 255  
subjectId| 查询参数所归属的主题ID| varchar| 255  
userId| 查询参数所归属的用户ID| varchar| 255  
  
  
## 17\. 加速引擎相关
### 17.1 fine_acceleration_query
BI开启加速引擎后记录的慢SQL
字段名称  
| 字段说明| 字段类型| 字段长度  
---|---|---|---  
id| 主键| varchar| 255  
categoryId| 仪表板ID| varchar| 255  
costTime| 耗时| bigint|   
  
detailMsg| 执行profiler| longtext|   
  
endTime| 结束时间| bigint|   
  
errorMsg| 报错信息| varchar| 255  
explainPlan| 执行计划| longtext|   
  
queryId| 查询ID| varchar| 255  
queryStatus| 查询状态| varchar| 255  
sqlStr| 执行SQL| longtext|   
  
startTime| 开始时间| bigint|   
  
subCategoryId| 组件ID| varchar| 255  
tableName| 表名称| varchar| 255  
userName| 用户名| varchar| 255  
### 17.2 fine_excel_ref
记录 Excel 数据与 SR 数据仓库中的表的关联。
字段名称  
| 字段说明| 字段类型  
| 字段长度  
---|---|---|---  
id| 主键| varchar| 255  
dbTableName| SR 数据仓库中表的名称| varchar| 255  
excelId| Excel表ID| longtext|   
  
excelOriginalName| Excel源文件名称| longtext|   
  
excelOriginalPath| Excel源文件路径| longtext|   
  
time| 记录时间| bigint|   
  
userName| 用户名| longtext|   
...
[![](https://help.fanruan.com/core/style/back.png)上一篇：配置库表结构-BI部分](<index.php?doc-view-2072.html>)
[下一篇：fine_conf_entity可视化配置 ![](https://help.fanruan.com/core/style/forward.png) ](<index.php?doc-view-1235.html>)
  * 有帮助
  * 没帮助
  * 只是浏览
