...
title: 日志库表结构
doc_id: 1134
url: https://help.fanruan.com/finebi/doc-view-1134.html
source: FineBI 7.X 帮助文档
crawled_at: 2026-07-16 17:30:31
version: "7.X"
---
...
* 历史版本：[74](<edition-list-1134.html>)
* 最近更新：[Carly](<user-space-222366.html>) 于 2026-05-19 
...
fine_record_dc_modelc_function_info| 数据中心用户使用表  
fine_tpl_info| 模板属性表  
...
displayName| 挂载到平台的目录名称| 挂载到平台的目录名称（不包括父目录等完整路径）如果直接通过链接访问则为空| VARCHAR  
...
userrole| 执行报表的用户身份(部门，角色等)| -| VARCHAR  
...
node| 加载模板的节点名| 需导出treasure包，详情请参见：[云端运维使用步骤](<https://help.fanruan.com/finebi7.0/doc-view-880.html>)| VARCHAR  
error| 模板执行过程中第一个报错信息| 需导出treasure包，详情请参见：[云端运维使用步骤](<https://help.fanruan.com/finebi7.0/doc-view-880.html>)| VARCHAR  
...
frFullVersion| FineReport版本|   
| VARCHAR  
biFullVersion| FineBI版本|   
| VARCHAR  
...
actionType| 操作类型0：登录1：注销2：超时登出3：保持登录4：强制登出| INTEGER  
...
## 15\. fine_record_dc_modelc_function_info 数据中心用户使用表
「数据目录&指标中心」下，该表用于记录核心功能（模型、搜索、数据预览、权限审批等）的用户使用情况，每个操作会产生一条记录。
在原驾驶舱中，该表用于统计资产被应用的情况，包括资产被应用于组件/仪表板的情况及次数、应用用户数及具体用户等。
完整字段说明如下：
字段名  
| 含义| 字段类型| 说明  
---|---|---|---  
uuid| 唯一标识字段| VARCHAR| -  
time| 记录产生时间| BIGINT| 到秒级（如2022-09-30 14:17:19）的时间戳   
appID| 系统应用ID| VARCHAR| 与分析无关，无需关注  
dcVersion| BI版本号| VARCHAR| -  
userName| 用户名| VARCHAR| 如sa、YJ201009069等，不是具体姓名  
actionType| 行为类型| VARCHAR| 字段较复杂，需要和assetType结合，见下表  
assetId| 资产id| VARCHAR| 可关联配置表分析  
assetType| 资产类型| VARCHAR| 
  * model：模型
  * BIData：BI基础数据
  * BIDataset：BI自助数据集
  * metric：指标
...
body| 主体内容| VARCHAR| 字段较复杂，需要和assetType结合，见下表  
actionType、assetType、body字段的对照说明如下：
所属模块  
| actionType| assetType| body  
---|---|---|---  
模型| 编辑-edit| model-模型、BIData-BI基础数据；BIDataset-BI自助数据集、metric-指标、dimension-维度、businessModel-业务模型、relationship-关联关系、folder文件夹| 
  * editSettings：name-资产名称、tag-标签、description-描述、calculation-计算公式、extendedField-拓展字段、selectedMetric-指标选择、selectedDimension-维度选择、relatedField-关联字段、relatedType-关联关系、analysisDirection-分析方向、enableRelation-关系启用
  * assetModule：dataCatalog-数据目录、modelCenter-模型中心

  
创建-create| model-模型、BIData-BI基础数据；BIDataset-BI自助数据集、metric-指标、dimension-维度、businessModel-业务模型、relationship-关联关系、folder文件夹| 
  * assetModule：dataCatalog-数据目录、modelCenter-模型中心

  
删除-delete| model-模型、relationship-关联关系|   
  
发布-publish| metric-指标、dimension-维度、businessModel-业务模型|   
  
搜索| 搜索检索-searchSearch|   
| 
  * searchWord：检索词
  * searchId：检索id
  * searchResults：检索条数
  * searchAccuracy：检索召回率，每次检索出现的搜索结果数/所有资产数量
  * searchPlace: dataCatalog-数据目录、modelManagement-模型管理、metricManagement-指标管理、dimensionManagement-维度管理、businessModel-业务模型
  * searchType：commonSearch-普通搜索、advancedSearch-增强搜索、AISearch-AI搜索

  
搜索后点击-clickResult| BIData-BI基础数据；BIDataset-BI自助数据集、metric-指标、dimension-维度、businessmodel-业务模型| 
  * searchWord：检索词
  * searchId：检索id
  * searchStep：检索步长，点击结果的排名。例如：1、2、999等
  * clickPeriod:输入搜索词到点击的时长。例如 1s
  * lastClickPeriod：距离上一次点击的时长。 例如 1s
  * clickTiming: before-筛选前、after-筛选后
  * searchType：commonSearch-普通搜索、advancedSearch-增强搜索、AISearch-AI搜索
...
| 
  ...
  * searchId：检索id
  * filterResults：筛选条数
  * filterClass：dataType-数据类型、tag-标签、createPeople-创建者、dataPermission-数据权限、updateTime-更新时间
  * searchType：commonSearch-普通搜索、advancedSearch-增强搜索、AISearch-AI搜索
...
| 
  * searchId：检索id
  * viewSource:显示top10的结果例如{资源ID、资源名称}
  * searchType：commonSearch-普通搜索、advancedSearch-增强搜索、AISearch-AI搜索
...
| 
  * searchId：检索id
  * applyType: create-创建分析主题、updateExcel-更新excel、edit-编辑、editSql-编辑SQL、add-添加到分析主题、permissionApply-权限申请
  * searchType：commonSearch-普通搜索、advancedSearch-增强搜索、AISearch-AI搜索
...
应用| 使用-apply| BIData-BI基础数据、metric-指标、dimension-维度、businessModel-业务模型、BIDataset-BI自助数据集| 
  * applyType：BIDataset-BI数据集、widget组件、dashboard仪表板、analyticalReport分析文档
  * applyId:应用资源的ID

  
列表| 列表调整-listAdjust|   
| 
  * adjustTiming: before-搜索前、after-搜索后
  * adjustWidth：调整宽度 例如 100px
  * listWidth：调整后的宽度 例如 100px

  
预览| 数据预览-previewClick|   
| 
  * previewType：dataPreview-数据预览、lineagePreview血缘预览、relatePreview相关性预览、infoPreview数据详情预览
  * lineageAction: search-血缘搜索、filter-血缘过滤、jump-血缘跳转、lineageInfo-血缘详情、upstreamClick-点击上游数量、downstreamClick-点击下游数数量
  * lineageFilter: updownStream-上下游、dataType-数据类型、permissionStatus-权限状态、publishStatus-发布状态、abnormalStatus-异常状态、createPeople-创建者

  
数据预览后分析-actionClick|   
| 
  * actionType：createProject-添加到分析主题、addToProject-添加到分析主题
...
审批-purviewPermission|   
| 
  * applyResults：申请结果，1-通过、0-拒绝

  
  

## 16\. fine_tpl_info 模板属性表
记录模板属性
...
## 17\. fine_container_entity 容器信息表
记录容器信息
**字段**| **含义**| **字段类型**  
---|---|---  
uuid| 唯一标识字段| VARCHAR  
...
item| 配置项| VARCHAR  
value| 配置项值| VARCHAR  
## 18\. fine_function_process 功能点信息表
记录功能点信息
**字段**| **含义**| **字段类型**  
---|---|---  
uuid| 唯一标识字段| VARCHAR  
...
## 19\. fine_operation_process_log 云端运维操作日志表
记录云端运维操作日志
**字段**| **含义**| **字段类型**  
---|---|---  
uuid| 唯一标识字段| VARCHAR  
...
## 20\. fine_operation_shutdown_record 信号量信息表
记录信号量信息
...
enable| 插件是否启用| BIT  
## 22\. fine_schedule_record 定时任务执行日志表
记录定时任务执行信息
**字段**| **含义**| **字段类型**  
---|---|---  
uuid| 唯一标识字段| VARCHAR  
...
taskID| 任务ID| VARCHAR  
time| 入库时间| BIGINT  
## 23\. fine_intelli_focus_point 功能埋点表
**功能埋点表：** 用来保存所有公共的埋点信息。
其各个字段对应的含义如下表所示：
**字段名**| **含义**| **数据格式**  
---|---|---  
...
## 24\. fine_real_time_usage 应用实时情况表
**应用实时情况表** ：记录应用实时情况。
其各个字段对应的含义如下表所示：
** 字段名**| **含义**| **数据格式**  
---|---|---  
...
## 25\. fine_intelli_consume_point 性能信息表
**性能信息表** ：记录性能相关信息。
其各个字段对应的含义如下表所示：
**字段名**| **含义**| **数据格式**  
---|---|---  
uuid| 唯一标识字段| String  
time| 时间开始时间| Long  
id| 埋点 id 编号BI 为 X961| String  
text| 资源内容可以是模板名字，分析表名字等。BI 未使用该字段| String  
source| 埋点来源Original 类里面embed：表示当前系统的plugin：表示插件的埋点undefined：表示没有定义来源| Int  
username| 操作者| String  
ip| 操作 IP| String  
title| 资源 ID可以是模板 ID，分析表 ID，表 ID 等| String  
...
## 27\. fine_config_operation 删除行为监控表
记录分组、业务包、表、关联的删除操作
**字段名**| **含义**| **数据格式**  
---|---|---  
uuid| 唯一标识字段| VARCHAR  
...
## 28\. fine_relationship 血缘关系分析表
记录数据表仪表板的血缘关系数据。
该表已弃用，如需查询可使用：[BI系统配置数据集](<https://help.fanruan.com/finebi7.0/doc-view-2149.html>)
**字段名**| **含义**| **数据格式**  
---|---|---  
uuid| 唯一标识字段| VARCHAR  
time| 时间| Long  
...
## 29\. fine_intelli_honeypot 即时埋点表
**字段名**| **含义**| **数据格式**  
---|---|---  
...
describe| 参数| VARCHAR  
## 30\. fine_intelli_log_honeypot 日志埋点表
**字段**| **含义**| **数据格式**  
---|---|---  
uuid| 唯一标识字段| VARCHAR  
id| ID| VARCHAR  
...
## 32\. fine_update_task_detail 更新任务明细表
...
## 33\. performAnalysis 数据结构
包括仪表板和数据准备两种结构。
注：该表目前已禁用，不会写入任何数据。如需查看数据表相关的SQL语句，请在「[BI工具](<https://help.fanruan.com/finebi7.0/doc-view-1546.html>)」的SQL性能监控中查看。
**字段名**| **含义**| **数据格式**  
---|---|---  
uuid| 唯一标识字段| String  
time|   
...
getFields| 获取字段信息  
## 34\. fine_engine_performance 计算任务基础信息记录
...
cloudVersion| 云端运维版本| VARCHAR  
## 35\. fine_sentinel_execution_record 预警运行监控埋点表
...
## 36\. fine_pretreat_job_result 日访问统计数据缓存表
...
[![](https://help.fanruan.com/core/style/back.png)上一篇：FineBI对接ElasticSearch实现日志存储服务](<index.php?doc-view-2675.html>)
[下一篇：FineBI对接华为云OBS实现数据存储服务 ![](https://help.fanruan.com/core/style/forward.png) ](<index.php?doc-view-2684.html>)
  * 有帮助
  * 没帮助
  * 只是浏览
